...
function Posts(props) {
  ...
  const unlikeHandler = (postId) => {
    ...
    Axios.post(`http://app:3001/unlike`, {
      post_id: postId,
      user_id: user.id,
    })
      .then((response) => {
  ...
  const deletePost = (postId) => {
    console.log(postId);
    Axios.get(`http://app:3001/delete/post/${postId}`)
      .then((response) => {
        console.log(response);
  ...
  const editPost = (e) => {
    Axios.post(`http://app:3001/edit/post/${postEdit.id}`, {
  ...
  const bookmarkHandler = (postId) => {
    console.log(postId);
    Axios.post(`http://app:3001/bookmark`, {
      post_id: postId,
      user_id: user.id,
    })
      .then((response) => {
  ...
  const unbookmarkHandler = (postId) => {
    Axios.post(`http://app:3001/unbookmark`, {
      post_id: postId,
      user_id: user.id,
    })
      .then((response) => {
  ...
  useEffect(() => {
    Axios.get(`http://app:3001/posts/${id}`).then((response) => {
    ...
    Axios.get(`http://app:3001/getLikes/${id}`).then((response) => {
      setLike(response.data);
      // console.log(like);
    }, []);
    Axios.get(`http://app:3001/getBookmarks/${id}`).then(
      (response) => {
