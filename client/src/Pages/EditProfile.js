...
function EditProfile() {
  ...
  //   });
  return (
    <motion.div className="h-screen w-[100%] grid md:grid-cols-2 grid place-items-center h-screen" initial={{opacity:0}}
    animate={{opacity:1}}
    exit={{opacity:0}}>
      <div>
        <div>
          <div className="h-70 w-70 bg-slate-300 rounded-full">
            <img className="h-70 w-70 bg-slate-300 rounded-full object-scale-down" src={user.profile_pic}></img>
          </div>
          <input
            type="file"
    ...
    "
        onChange={profileImageHandler}
          />
          <button className="border-2" onClick={uploadImage}>Upload</button>
        </div>
      </div>
